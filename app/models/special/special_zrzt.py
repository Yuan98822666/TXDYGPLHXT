...
class SpecialZrzt(Base):
    __tablename__ = "special_zrzt"
    ...
    yfbt = Column(Integer, comment="首次封板时间（HHMMSS）")
    ylbc = Column(Integer, comment="炸板次数")
    ...
    __table_args__ = (
        Index("ix_special_zrzt_code_date", "stock_code", "trade_date", unique=True),
        {"comment": "昨日涨停池表"},
    )

    def __repr__(self):
        return f"<SpecialZrzt code={self.stock_code} name={self.stock_name}>"
