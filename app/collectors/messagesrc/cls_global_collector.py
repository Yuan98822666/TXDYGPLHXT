...
"""
财联社环球消息采集器
API: https://www.cls.cn/v3/depth/home/assembled/1007
"""
...
class CLSGlobalCollector:
    """财联社环球消息采集器"""
    
    def __init__(self):
        self.base_url = "https://www.cls.cn/v3/depth/home/assembled/1007"
        self.headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
            'Accept': 'application/json, text/plain, */*',
            'Referer': 'https://www.cls.cn/depth?id=1007',
        }
    ...
    def fetch_articles(self, mode: str = "incremental") -> tuple:
        ...
        try:
            ...
            logger.info(f"开始采集环球消息 [mode={mode}]")
            ...
            logger.info(f"解析到 {len(articles)} 条环球消息")
    ...
    def _save_articles(self, articles: List[Dict[str, Any]]) -> int:
        ...
        with get_db_context() as db:
            for article in articles:
                try:
                    # 检查是否已存在
                    existing = db.query(MessageSrcCLSGlobal).filter(
                        MessageSrcCLSGlobal.article_id == article["article_id"]
                    ).first()
        ...
        logger.info(f"成功保存 {inserted} 条新环球消息")
        return inserted
    ...
    def _get_last_article_time(self) -> datetime:
        ...
        with get_db_context() as db:
            latest = db.query(MessageSrcCLSGlobal).order_by(
                MessageSrcCLSGlobal.publish_time.desc()
            ).first()
...
# 全局实例
global_collector = CLSGlobalCollector()
